 ...
 * @author chenpeng
 */
public class HungryManJavaSerializeSingleton implements Serializable {
     ...
     */
    public static final HungryManJavaSerializeSingleton SINGLETON = new HungryManJavaSerializeSingleton();

    /**
     * 私有化构造方法 防止外部类通过new的方式实例化对象
     */
    private HungryManJavaSerializeSingleton() {
        if (SINGLETON != null) {
    ...
    /**
     * readResolve方法的作用为防止序列化单例时破坏唯一实例的规则 在序列化时进行了readResolve的判断
     */
